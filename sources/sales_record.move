module sales_record::sales_record{
    ...
    public fun get_record_length(account_address : address) : u64 acquires Records {
        ...
        assert!(exists<Records>(account_address), error::invalid_state(ERECORDS_NOT_EXISTS)); // throws error if records does not exists
    ...
    public fun get_record_by_index(account_address : address, index : u64) : (string::String, string::String, u64) acquires Records {
        ...
        assert!(index < vector::length<Record>(records), error::invalid_state(EINDEX_OUT_OF_RANGE)); // checks if index is within records range
    ...
    public fun create_records_if_not_exists(account : &signer) {
        ...
        assert!(!exists<Records>(account_address), error::invalid_state(ERECORDS_NOT_EXISTS)); // throws error if records exists
